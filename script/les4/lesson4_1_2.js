...
let addExpenses = prompt("Перечислите возможные расходы за рассчитываемый период через запятую").replace(" ", "").split(",");



function getExpensesMonth(addExpenses, size){
    if(size>0){
        return (addExpenses[size-1]) +getExpensesMonth(addExpenses, (size-1));
    }else return 0;
